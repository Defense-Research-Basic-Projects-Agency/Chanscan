...
var triggers = [/caroline/, /trapan/, /squishy/, /large john knickers/, /arceus/, /discord/, /KjFYwd9/]
...
for (i in boards)
{
  astoria.board(boards[i])
  	.listen((context, threads, err) => {
        ...
        console.log(`[THREAD] https://boards.4chan.org/${boards[i]}/thread/${thread.no}: ${entities.decode(striptags(thread.com))}`)
        var trig = triggerParse(thread.com)
        if (trig > 0) console.log(trig)
        let unsubscribe = astoria.board(boards[i])
      	...
      	.listen((context, posts, err) => {
            ...
            console.log(`[POST] https://boards.4chan.org/${boards[i]}/thread/${thread.no}: ${entities.decode(striptags(post.com))}`)
